...
const handleSignup = async () => {
  ...
  try {
    ...
    toast.error(err.statusMessage || 'Registration failed. Please try again.')
  } finally {
    ...
    <div class="relative z-10 w-full max-w-4xl bg-slate-900 border border-slate-800 rounded-2xl shadow-md overflow-hidden grid grid-cols-1 md:grid-cols-2">
      ...
      <div class="hidden md:flex flex-col justify-between p-10 bg-gradient-to-br from-slate-900 via-purple-950/20 to-slate-900 border-r border-slate-800 relative">
        ...
        <div class="space-y-4">
          ...
          <p class="text-slate-400 text-sm leading-relaxed">
            Gain full control over telegram broadcast schedules, channels, active automated moderation, and view performance logs in real-time.
          </p>
        </div>
      ...
      <div class="p-8 md:p-10 flex flex-col justify-center bg-slate-900/40">
        ...
        <div class="mb-6">
          ...
          <p class="text-xs text-slate-400 mt-1">Register a new dashboard administrator</p>
        </div>
        ...
        <form @submit.prevent="handleSignup" class="space-y-4">
          ...
          <div>
            <label class="block text-[11px] font-semibold uppercase tracking-wider text-slate-400 mb-1.5">Confirm Password</label>
            <div class="relative">
              <Lock class="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500" />
              <input
        ...
        <div class="mt-8 text-center">
          <p class="text-xs text-slate-500">
            ...
            <NuxtLink to="/login" class="text-purple-400 hover:text-purple-300 font-semibold transition-colors ml-1">
              Sign In
            </NuxtLink>
          </p>
        </div>
